...
compareToCory <- function()
{
   ...
   pauls.ranks <- match(gene.models[["mef2c.tcx"]]$gene[1:10], tfs.0)
   corys.ranks <- match(tfs.0[1:10], gene.models[["mef2c.tcx"]]$gene[1:10])
   tbl.tcx <- data.frame(cory=gene.models[["mef2c.tcx"]]$gene[1:10],
                         pauls.rank=pauls.ranks,
                         paul=tbl.trn.0$tf[1:10],
                         corys.rank=corys.ranks)
   tbl.tcx

    #--------------------------------------------------------------------------------
    # use mtx.cer, pre-calculated footprints, cory's 10kb region, TFclass mapping
    #--------------------------------------------------------------------------------
   ...
   pauls.ranks <- match(gene.models[["mef2c.cer"]]$gene[1:10], tfs.1)
   corys.ranks <- match(tfs.1[1:10], gene.models[["mef2c.cer"]]$gene[1:10])
   tbl.cer <- data.frame(cory=gene.models[["mef2c.cer"]]$gene[1:10],
                         pauls.rank=pauls.ranks,
                         paul=tbl.trn.1$tf[1:10],
                         corys.rank=corys.ranks)
   tbl.cer
   ...
   checkEquals(sort(names(x1)), c("bindingSites", "trn"))
   tbl.trn.2 <- x1$trn
   tbl.bindingSites.2 <- x1$bindingSites
   tfs.2 <- tbl.trn.2$tf
   pauls.ranks <- match(gene.models[["mef2c.ros"]]$gene[1:10], tfs.2)
   corys.ranks <- match(tfs.2[1:10], gene.models[["mef2c.ros"]]$gene[1:10])
   tbl.ros <- data.frame(cory=gene.models[["mef2c.ros"]]$gene[1:10],
                         #paul=tbl.trn.2$tf[hits],
                         pauls.rank=pauls.ranks,
                         paul=tbl.trn.2$tf[1:10],
                         corys.rank=corys.ranks)
...
{
   tbl.elite <- read.table("geneHancer/enhancer_elite_ids.txt", sep="\t", as.is=TRUE, header=TRUE)            #  243281      6
   tbl.geneScoresAll <- read.table("geneHancer/enhancer_gene_scores.txt", sep="\t", as.is=TRUE, header=TRUE)  #  934287      9
